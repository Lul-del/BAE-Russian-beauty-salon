...
const slides = [
  { src: 'https://images.unsplash.com/photo-1633681926022-84c23e8cb2d6?w=1920&h=1080&fit=crop', alt: 'BAE Beauty Salon' },
  ...
  return (
    <section id="accueil" className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 text-center px-3 sm:px-6 max-w-5xl mx-auto w-full">
        ...
        <h1 className="font-display text-5xl sm:text-7xl md:text-8xl lg:text-9xl font-bold text-white mb-3 sm:mb-4 slide-up">BAE</h1>
        ...
        <div className="flex items-center justify-center gap-2 sm:gap-4 mb-4 sm:mb-6 slide-up [animation-delay:0.2s]">
          ...
          <span className="text-rose-300 dark:text-[#D4AF37] font-display text-base sm:text-xl italic whitespace-nowrap">{t.hero.tagline}</span>
        ...
        <p className="text-sm sm:text-base text-white/50 max-w-xl mx-auto mb-8 sm:mb-10 slide-up [animation-delay:0.5s]">
          {t.hero.desc}
        </p>

        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 slide-up [animation-delay:0.6s]">
          <a href="#contact"
            className="w-full sm:w-auto px-6 sm:px-8 py-3 sm:py-4 rounded-full bg-linear-to-r from-[#FF61EF] to-[#ED5389] dark:from-[#D4AF37] dark:to-[#D4AF37] text-white font-semibold text-base sm:text-lg shadow-2xl shadow-rose-500/30 hover:shadow-rose-500/50 dark:shadow-yellow-500/30 transition-all hover:scale-105 flex items-center justify-center gap-2">
        ...
        <div className="mt-10 sm:mt-16 flex items-center justify-center gap-4 sm:gap-8 text-white/60 slide-up [animation-delay:0.8s] flex-wrap">
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <span className="text-xs sm:text-sm">{t.hero.rating ?? '4.9/5'}</span>
          </div>
          <div className="w-px h-4 sm:h-5 bg-white/20" />
          <span className="text-xs sm:text-sm">{t.hero.clients}</span>
          <div className="w-px h-4 sm:h-5 bg-white/20 hidden sm:block" />
          <span className="text-xs sm:text-sm hidden sm:block">{t.hero.location}</span>
        </div>
      </div>
      ...
      <a href="#services" aria-label="Scroll" className="absolute bottom-6 sm:bottom-8 left-1/2 -translate-x-1/2 text-white/50 animate-bounce">
